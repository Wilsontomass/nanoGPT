...
class RotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000):
        ...
        # Precompute cos and sin terms to save computation during forward pass
    ...
    def forward(self, x, seq_len=None):
        # Return cached cos and sin embeddings
        seq_len = seq_len or x.size(1)
        if seq_len > self.max_seq_len_cached:
            raise ValueError(f"Sequence length {seq_len} exceeds maximum {self.max_seq_len_cached}")
        return (
...
def apply_rotary_pos_emb(x, cos, sin):
    # x: (batch_size, seq_len, num_heads, 2 * head_dim)
    x1 = x[..., ::2]  # Even indices
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h = nn.ModuleList([Block(config, depth+1) for depth in range(config.n_layer)]),
